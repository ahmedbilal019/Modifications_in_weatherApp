...
export default function Card() {
    ...
    const weatherIconUrl = data.weather ? `http://openweathermap.org/img/wn/${data.weather[0].icon}@2x.png` : null;
    return (
       
        <div className='card'>
            <div className='header'>
                <input type="text" name="searchbar" className='search-field' id="search" placeholder='Enter City name...' onChange={handlechangeInput} value={cityName} />
                <button className='searchbtn' onClick={handleSearch}> <img src={searchIcon} alt="Search Icon" srcset="" className='searchIcon' /></button>
            </div>
